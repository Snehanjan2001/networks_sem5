...
class VRC:
    ...
    def encode(binaryInputString: str, dataWordFrameSize: int):
        # ? Even Parity VRC
        output = ""
        for i in helpers.buildFrames(binaryInputString, dataWordFrameSize):
            _, noOfOnes = helpers.ReadNoOfZerosAndOnes(i)
            output += i + ("1" if noOfOnes % 2 == 1 else "0")
        return output

    @staticmethod
    def decode(binaryInputString: str, dataWordFrameSize):
        frames = helpers.buildFrames(binaryInputString, dataWordFrameSize + 1)
        errorFound = False
        output = ""
        for i in frames:
            # ? Even Parity VRC
...
class LRC:
    ...
    def encode(binaryInputString: str, dataWordFrameSize: int, noOfOriginalDataFramesPerGroup: int = 4):
        output = ""
        frames = helpers.buildFrames(binaryInputString, dataWordFrameSize)
    ...
    def encode(binaryInputString: str, dataWordFrameSize: int, noOfOriginalDataFramesPerGroup: int = 4):
        ...
        for i in range(0, len(frames), noOfOriginalDataFramesPerGroup):
            ...
            for frame in tmpFrames[1:]:
                tmp = helpers.addBinaryStringUsingOnesComplement(tmp, frame)
                frameString += frame

            # Complement and get checksum
            checksum = helpers.complementOfBinaryString(tmp)
...
class CRC:
    ...
    def encode(binaryInputString: str, dataWordFrameSize: int, divisor: str):
        ...
        for i in helpers.buildFrames(binaryInputString, dataWordFrameSize):
            ...
            crc = helpers.divisonCRC(tmp, divisor)[:crcSize]
    ...
    def decode(binaryInputString: str, dataWordFrameSize: int, divisor: str):
        ...
        errorFound = False
        crcSize = len(divisor) - 1
        for i in helpers.buildFrames(binaryInputString, dataWordFrameSize + crcSize):
            remainder = helpers.divisonCRC(i, divisor)
