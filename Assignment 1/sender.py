...
from helpers import StringToBinary, generateDivisor, polynomialStringParser
from error_detection import CRC, VRC, LRC, CheckSum
...
class Sender:
    ...
    def injectErrorInOutput(self, loopC=1):
        for i in range(loopC):
            ...
            self.output = self.output[:random_bit_location] + (
                '0' if self.output[random_bit_location] == '1' else '1') + self.output[random_bit_location + 1:]
...
if __name__ == "__main__":
    ...
    if encodingMethod not in ["VRC", "LRC", "CRC", "CHECKSUM"]: raise Exception(
        "Invalid encoding method")  # Check for invalid input
    selectedPolynomial = ""
    ...
    if encodingMethod == "VRC":
        ...
        noOfOriginalDataFramesPerGroup = 4 if noOfOriginalDataFramesPerGroup == '' else int(
    ...
    print("Encoding technique     : ",
          encodingMethod + " " + selectedPolynomial if encodingMethod == "CRC" else encodingMethod)
